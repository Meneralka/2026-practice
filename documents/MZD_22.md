...
* Карточка документа во время автопроверок находится в статусе `DRAFT` — технический результат несёт `FileVersion.status`, статус карточки его не дублирует (Q-1, close-questions). Переход `DRAFT` -> `UNDER_REVIEW` автоматический при `CLEAN` + валидном контексте бизнес-объекта; на бизнес-стадии разрешен только для файлов в статусе `CLEAN`.
